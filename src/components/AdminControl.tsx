  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "admin_notifications" }, (payload) => {
        ...
        toast.info("New System Notification", { description: payload.new.title });
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles", filter: "is_approved=eq.false" }, () => {
        fetchData();
      })
      .subscribe();
                    ...
                    >
                      ...
                      <span className="text-[10px] font-bold tracking-widest uppercase leading-none relative z-10">{tab.label}</span>
                          {tab.id === "notifications" && unreadNotificationsCount > 0 && (
                            <span className="ml-auto bg-red-500 text-white text-[9px] font-black px-2 py-0.5 rounded-full relative z-10">{unreadNotificationsCount}</span>
                          )}
...
{activeTab === "requests" && (
                ...
                {activeTab === "notifications" && (
                  <motion.div key="notifications" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col">
                    <div className="flex justify-between items-center mb-8 shrink-0">
                      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Recent System Logs</p>
                      <Button 
                        onClick={markNotificationsRead}
                        disabled={unreadNotificationsCount === 0}
                        variant="ghost" 
                        className="h-10 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest px-6"
                      >
                        <CheckCircle className="w-3 h-3 mr-2" /> Mark All Read
                      </Button>
                    </div>
                    ...
                    {adminNotifications.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-20 bg-white/[0.01] border border-white/5 border-dashed rounded-[3rem]">
                        ...
                        <p className="text-sm font-black italic text-white/40 uppercase tracking-widest">No system alerts captured</p>
                      </div>
                    ) : (
                      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-2 space-y-4 pb-8">
                        {adminNotifications.map((notification) => (
                          <div key={notification.id} className={`p-6 border rounded-2xl flex items-start gap-6 transition-all ${notification.is_read ? 'bg-white/[0.01] border-white/5 opacity-60' : 'bg-indigo-500/5 border-indigo-500/20 shadow-lg shadow-indigo-500/5'}`}>
                            <div className={`p-3 rounded-xl ${notification.type === 'new_registration' ? 'bg-orange-500/20 text-orange-400' : 'bg-indigo-500/20 text-indigo-400'}`}>
                              <Bell className="w-5 h-5" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-4 mb-2">
                                <h4 className="font-black uppercase italic tracking-tight text-white">{notification.title}</h4>
                                <span className="text-[8px] font-bold text-white/20 uppercase tracking-widest">{new Date(notification.created_at).toLocaleString()}</span>
                              </div>
                              <p className="text-sm text-white/60 leading-relaxed mb-4">{notification.content}</p>
                              {notification.type === 'new_registration' && (
                                <Button 
                                  onClick={() => setActiveTab("requests")}
                                  className="h-10 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[9px] font-black uppercase tracking-widest px-6"
                                >
                                  View Request <ArrowUpRight className="w-3 h-3 ml-2" />
                                </Button>
                              )}
                            </div>
                            {!notification.is_read && <div className="w-2 h-2 bg-indigo-500 rounded-full mt-2 animate-pulse" />}
                          </div>
                        ))}
                      </div>
                    )}
