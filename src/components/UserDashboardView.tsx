...
export function UserDashboardView({ session, privateKey }: UserDashboardViewProps) {
  ...
  const [friendProfiles, setFriendProfiles] = useState<any[]>([]);
    const [advancedSubView, setAdvancedSubView] = useState<"menu" | "vault" | "memories" | "watch">("menu");
  ...
  const [selectedFriendForWatch, setSelectedFriendForWatch] = useState<any>(null);
        ...
        if (data.type === "offer" && !activeCall && !incomingCall) {
          const { data: caller } = await supabase.from("profiles").select("*").eq("id", data.caller_id).single();
            if (caller) {
              ...
              const modeDisplay = `${data.call_mode} call`;
              toast.info(`Incoming ${modeDisplay} from ${caller.username}`, { duration: 10000 });
              showNotification(`Incoming ${modeDisplay}`, { body: `Call from ${caller.username}` });
              addNotification("call", `Incoming ${modeDisplay}`, `${caller.username} is calling you`, { callerId: data.caller_id });
            }
        }
      }).subscribe();
  ...
  const handleNavClick = (view: ActiveView) => {
    ...
    if (view === "advanced") {
      ...
      setSelectedFriendForWatch(null);
    }
  ...
  const unreadNotificationCount = notifications.filter(n => !n.read).length;
    ...
    const navItems = [
      ...
      const advancedFeatures = [
            ...
            { id: "memories", icon: CalendarHeart, label: "Memories", desc: "Special days calendar", color: "from-orange-600 to-amber-600" },
            { id: "watch", icon: Tv, label: "Watch Together", desc: "Watch movies with friends", color: "from-rose-600 to-pink-600" },
            { id: "social", icon: Film, label: "Social Nexus", desc: "YouTube Shorts & Instagram", color: "from-red-600 to-pink-600" },
          ];
            ...
            {activeView === "advanced" && (
              <motion.div key="advanced" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="h-full overflow-y-auto custom-scrollbar">
                      ...
                      <div className="mb-8">
                        ...
                        <p className="text-sm text-white/40">Access premium features with your friends</p>
                          ...
                          <div className="mb-10 flex justify-center">
                            <div className="max-w-4xl w-full space-y-6 bg-zinc-900/50 p-6 sm:p-10 rounded-[2.5rem] sm:rounded-[3.5rem] border border-zinc-800 backdrop-blur-3xl relative overflow-hidden shadow-2xl group">
                ...
                {advancedSubView === "memories" && selectedFriendForMemories && (
                  ...
                  {advancedSubView === "watch" && !selectedFriendForWatch && (
                    <div className="p-6 sm:p-8 pb-32 lg:pb-12">
                      <Button variant="ghost" onClick={() => setAdvancedSubView("menu")} className="mb-6 text-white/40 hover:text-white">
                        <ChevronRight className="w-4 h-4 rotate-180 mr-2" /> Back to Advanced
                      </Button>
                      <div className="mb-8">
                        <h2 className="text-2xl font-black uppercase italic mb-2">Watch Together</h2>
                        <p className="text-sm text-white/40">Select a friend to watch movies together</p>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {friendProfiles.map(p => (
                          <motion.div key={p.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                            onClick={() => setSelectedFriendForWatch(p)}
                            className="p-6 bg-gradient-to-br from-rose-900/20 to-pink-900/20 border border-white/5 rounded-3xl flex flex-col items-center gap-4 hover:border-rose-500/30 transition-all cursor-pointer group">
                            <div className="relative">
                              <AvatarDisplay profile={p} className="h-16 w-16 group-hover:scale-105 transition-transform" />
                              <div className="absolute -bottom-1 -right-1 p-1.5 bg-rose-600 rounded-full">
                                <Tv className="w-3 h-3 text-white" />
                              </div>
                            </div>
                            <div className="text-center">
                              <p className="font-black text-lg uppercase">{p.username}</p>
                              <p className="text-[10px] font-bold uppercase tracking-widest text-rose-400">Watch Together</p>
                            </div>
                          </motion.div>
                        ))}
                      </div>
                    ...
                    {advancedSubView === "watch" && selectedFriendForWatch && (
                      <div className="h-full">
                        <WatchTogether
                          userId={session.user.id}
                          friendId={selectedFriendForWatch.id}
                          friendProfile={selectedFriendForWatch}
                          myProfile={myProfile}
                          onClose={() => setSelectedFriendForWatch(null)}
                        />
                      </div>
                    )}

                    {advancedSubView === "social" && (
                      <div className="h-full">
                        <SocialFeed
                          onClose={() => setAdvancedSubView("menu")}
                        />
                      </div>
                    )}
                </motion.div>
