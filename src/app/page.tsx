...
import { Lock, Shield, Zap, Globe, MessageSquare, Terminal, Activity, Fingerprint, Flame, ShieldAlert, Network, Box, RefreshCw } from "lucide-react";
...
export default function Home() {
  ...
  const [isKeySetupLoading, setIsKeySetupLoading] = useState(false);
  ...
  async function checkApprovalAndOTP(userId: string) {
    try {
      const { data, error } = await supabase
        .from("profiles")
        .select("is_approved, otp_enabled, is_admin")
        .eq("id", userId)
        .single();
      ...
      if (error) {
        ...
        if (data.otp_enabled && (data.is_approved !== false || data.is_admin)) {
          setOtpRequired(true);
  ...
  async function handleKeySetup() {
    try {
      ...
      if (storedPrivKey && storedPrivKey !== "undefined" && storedPrivKey !== "null" && storedPubKey && storedPubKey !== "undefined") {
        try {
          const privKey = await importPrivateKey(storedPrivKey);
          
          // Verify if our local identity matches the server
          if (profile?.public_key && storedPubKey !== profile.public_key) {
            console.warn("Local identity mismatch with server. Synchronizing keys...");
            await supabase.from("profiles").update({ 
              public_key: storedPubKey,
              updated_at: new Date().toISOString()
            }).eq("id", session.user.id);
          } else if (!profile?.public_key) {
            // Server missing key, upload local
            await supabase.from("profiles").update({ 
              public_key: storedPubKey,
              updated_at: new Date().toISOString()
            }).eq("id", session.user.id);
          ...
          console.error("Key import failed, generating new one", e);
      ...
      setKeyError(true);
      toast.error("Encryption system error. Please refresh.");
    } finally {
      setIsKeySetupLoading(false);
    }
  }
  ...
  async function generateAndStoreNewKey() {
    try {
      ...
      console.error("Failed to generate and store new key:", e);
      setKeyError(true);
    } finally {
      setIsKeySetupLoading(false);
    }
  ...
  if (session && (isKeySetupLoading || (isApproved && !otpRequired && !privateKey && !keyError))) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#010101] overflow-hidden">
        <div className="relative">
          <div className="absolute inset-0 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 blur-[150px] opacity-30 animate-pulse rounded-full" />
          ...
          <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 w-48 text-center">
              <p className="text-[10px] font-black uppercase tracking-[0.5em] text-white/30">Syncing Identity</p>
          </div>
        </div>
      </div>
    );
  ...
  if (session && isApproved === false) {
    return (
      <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-[#010101] p-8 text-center relative overflow-hidden">
        ...
        <div className="absolute bottom-[-20%] right-[-20%] w-[60%] h-[60%] bg-orange-900/10 blur-[200px] rounded-full pointer-events-none" />
        ...
        <motion.div 
          ...
          <div className="space-y-6">
            <h2 className="text-5xl font-black italic tracking-tighter text-white uppercase">Access <span className="text-orange-500">Pending</span></h2>
            <p className="text-zinc-500 font-medium leading-relaxed tracking-widest text-xs uppercase">
              System protocols require administrative clearance. Your node activation is currently awaiting manual verification by command core.
            </p>
          </div>
  ...
  if (session && keyError) {
    return (
      <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-[#010101] p-8 text-center relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="z-10 space-y-8 max-w-md">
          <div className="p-8 bg-white/5 border border-white/10 rounded-[2.5rem] backdrop-blur-3xl">
            <ShieldAlert className="w-12 h-12 text-indigo-400 mx-auto mb-6" />
            <h3 className="text-xl font-black uppercase italic mb-4">Identity System Error</h3>
            <p className="text-zinc-400 text-xs uppercase tracking-widest leading-relaxed mb-8">
              We encountered a critical failure initializing your encryption keys. This protocol is required for secure communication.
            </p>
            <Button 
              onClick={() => window.location.reload()}
              className="w-full bg-indigo-600 hover:bg-indigo-700 py-6 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px]"
            >
              <RefreshCw className="w-4 h-4 mr-2" /> Reboot Kernel
            </Button>
          </div>
          <button onClick={() => supabase.auth.signOut()} className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 hover:text-white transition-colors">
            Terminate Session
          </button>
        </motion.div>
      </div>
        ...
        {!session ? (
          ...
          (privateKey || keyError) && (
              <motion.div
                ...
                className="h-full"
              >
                <UserDashboardView session={session} privateKey={privateKey || (null as any)} />
              </motion.div>
            )
        )}
      </AnimatePresence>
